...
-- number of cities in latitude region
WITH new_cities AS
	(SELECT name, 
	CASE 
...
-- number of cities in longitude region
...
-- number of cities per countries

SELECT c.name as country_name, COUNT(c.name) as num_cities
FROM cities t JOIN countries c
	ON t.country_id = c.id
GROUP BY c.name
ORDER BY num_cities DESC

-- number of cities per states

SELECT s.name as states_name, COUNT(s.name) as num_cities
FROM states s JOIN cities t
	ON t.state_id = s.id
GROUP BY s.name
ORDER BY num_cities DESC
...
SELECT c.region as region_name, COUNT(c.region) as num_cities
FROM cities t JOIN countries c
	ON t.country_id = c.id
GROUP BY c.region
ORDER BY num_cities DESC

-- number of cities per subregion

SELECT c.subregion as subregion_name, COUNT(c.subregion) as num_cities
FROM cities t JOIN countries c
	ON t.country_id = c.id
